...
namespace Flower{
    ...
    public class FlowerSystem : MonoBehaviour
    {
        ...
        private void Awake() {
           ...
           RegisterCommand("start", (List<string> _params) => { dialog.SetActive(true); });
            RegisterCommand("end", (List<string> _params) => { dialog.SetActive(false); });
       ...
       public void SetupDialog(string resourcePath="DefaultDialogPrefab", bool applyDefaultTextUpdateEvent=true, bool applyDefaultTextSplitting=false){
            try{
                var dialogPrefab = LoadResource<GameObject>(resourcePath);
                 dialog = CreateAsSceneObject("_Dialog", dialogPrefab, Vector3.zero, false);
                if(applyDefaultTextUpdateEvent){
                    try{
                        ...
                        try
                        {
                            dialog_text = dialog.transform.Find("DialogPanel (1)/DialogText (1)").GetComponent<Text>();
                            Dialog_Image = dialog.transform.Find("Image/Image").GetComponent<Image>();
                        }
                        catch
                        {
